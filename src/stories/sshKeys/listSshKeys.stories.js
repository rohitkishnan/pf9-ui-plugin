...
import SshKeysList from 'openstack/components/sshkeys/SshKeysList'
...
addStories('SSH Key Management/Listing keys', {
  'With no keys': () => (
    <SshKeysList sshKeys={[]} onAdd={addAction} onDelete={deleteAction} />
  ),

  'With some keys': () => (
    <SshKeysList sshKeys={someSshKeys} onAdd={addAction} onDelete={deleteAction} />
  ),
  ...
  'With pagination': () => {
    ...
    return (
      <SshKeysList sshKeys={sshKeys} onAdd={addAction} onDelete={deleteAction} />
    )
  },
})
